    ...
    fetch("http://localhost:8080/reviews/api/user")
        ...
        .then(reviews => {
                if (reviews.length === 0) {
                    userReviewsContainer.innerHTML =
                        `<div class="mt-3">
                            <h5 class="text-left text-secondary">You have no reviews yet or reviews are pending approval.</h5>
                        </div>`;
                    return;
                }
...
function createBookReviewElement(review) {
    ...
    article.innerHTML = `<div class="card-body">
                            <h4 class="card-title">
                                <strong class="font-weight-bold border-bottom border-dark">
                                    ${review.title}
                                </strong>
                            </h4>
...
function createUserReviewElement(review) {
    ...
    article.innerHTML = `<h4 class="card-header">
